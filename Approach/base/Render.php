...
class renderable
{
	...
	public $content=null;		//If content and children both null, tag is <selfcontained />
	...
	function renderable($t='div', $pageID='', $options=array())
	{
		...
		if(isset($options['pageID']) )	{ $this->pageID = $options['pageID']; $pageID = $this->pageID; }
		...
		if(in_array($this->tag,renderable::$NoAutoRender) || $pageID == NULL) $this->pageID='';
	}
	...
	public function buildClasses()
	{
		...
		elseif(in_array($this->tag,renderable::$NoAutoRender) || $this->pageID==NULL){ return ''; }
		else
		{
...
//		 if($this->attributes === 0){ $this->attributes = '';	 }
	...
	public function render()
	{
		...
		$open_open = '<';
		$open_close = $this->selfContained ? ' />'.PHP_EOL : '>';
		$close_open = '</';
		$close_close = '>';

		if($this->tag === NULL || $this->tag === '')
			$this->tag = $open_open = $open_close = $close_open = $close_close ='';

		$OutputStream = $this->preFilter.$open_open .
			// Opening Tag Or Decoration
		 	$this->tag .
			...
			// Content
			$FrontendMarkup . $this->content . $this->lastFilter .

			// Closing Tag Or Decoration
			$close_open . $this->tag . $close_close .
			$this->postFilter.PHP_EOL;
    ...
    public function copyInto(&$into,$level=255)
    {
        switch ($level)
        {
            case 255: $this->full_copyInto($into); break;
            ...
            case 1: $this->shallow_copyInto($into); break;
            default: $this_>full_copyInto($into); break;
        }
        return $this;
    }
    ...
    {
        $into = clone $this;
        $into->id=renderable::$renderObjectIndex;
		renderable::$renderObjectIndex++;				/*	Register New Renderable	*/
    }
}
