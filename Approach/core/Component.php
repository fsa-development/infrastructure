	...
	function Process(&$BuildData)
	{
        ...
        for($b=0,$BL=count($BuildData); $b<$BL; ++$b)// as $ConsolidatedRow)
		{
            ...
            for($i=0, $L=$TemplateCount; $i<$L; ++$i)
    		{
				...
				$SmartObject = new $this->RenderType(array('tag'=>$this->ChildTag[$i],'template' => $this->context['template'], 'markup' => $i) );
                ...
                $this->ParentContainer->children[] = $SmartObject;
	...
	function Save($IncomingTokens, $TemplateBinding)
	{
		...
		foreach($TemplateBinding as $Component => $_Dataclass)
		{
			...
			foreach($_Dataclass as $Name => $PropertyList)
			{
				...
				{
					require_once($DataPath.'/'. $Name . '.php');
					$dbo = new $Name($Name);
